...
func TestConfigRejectsUnsafeIdentityAndPaths(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "compute agent path control", mutate: func(c *Config) { c.ComputeAgentPath += "\nnext" }, want: "canonical safe path"},
		{name: "compute agent path noncanonical", mutate: func(c *Config) { c.ComputeAgentPath = home + "/bin/../compute-agent" }, want: "canonical safe path"},
		{name: "supervisor config path control", mutate: func(c *Config) { c.SupervisorConfigPath += "\x7f" }, want: "canonical safe path"},
		{name: "local status path control", mutate: func(c *Config) { c.LocalStatusPath += "\rnext" }, want: "canonical safe path"},
		{name: "provider marker path control", mutate: func(c *Config) { c.ProviderMarkerPath += "\tnext" }, want: "canonical safe path"},
		{name: "install root path control", mutate: func(c *Config) { c.InstallRoot += "\x00next" }, want: "canonical safe path"},
		{name: "systemd path control", mutate: func(c *Config) { c.SystemdDir += "\x1fnext" }, want: "canonical safe path"},
